...
/**
 * 类<code>ScrollOverListView</code>带有下拉刷新的ListView<br>
 * <br>
 * <br>
...
public class ScrollOverListView extends ListView implements OnScrollListener {
	...
	private void findView(Context context) {
		...
		headView = (LinearLayout) inflater.inflate(R.layout.model_pull_listview_head, null);
		...
		layoutContent = (RelativeLayout) headView.findViewById(R.id.layoutContent);
	...
	public void setHeader(View header) {
		...
		if (null == this.header) {
			...
			throw new IllegalStateException("header is not null");
		}

		// measureView(layoutContext);
	}

	/**
	 * 
	 * 函数名称 : removeHeader 功能描述 : 移除头部 参数及返回值说明：
	 * 
	 * 修改记录： 日期：2012-12-5 上午11:57:26 修改人：vendor 描述 ：
	 * 
	 */
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN: { // 按下的时候
			...
			if (firstItemIndex == 0 && !isRecored) {
				...
				startY = (int) ev.getY();
				Log.v(TAG,
						"在down时候记录当前位置: X: " + ev.getX() + " Y: " + ev.getY());
			}
			// ===========================
			mLastY = y;
	...
	public void onScrollStateChanged(AbsListView view, int scrollState) {
		if(scrollState == OnScrollListener.SCROLL_STATE_IDLE && isRefreshFoot){  
			mOnScrollOverListener.onListViewBottomAndPullUp(-10);
		}
	}
